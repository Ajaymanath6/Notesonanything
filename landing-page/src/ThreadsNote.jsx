...
// Yellow-themed Threads Note Component with Comments
const ThreadsNote = ({ 
  content = "",
  author = "You",
  avatar = "YO",
  ...
  return (
    ...
    >
      {/* Main Note Container */}
      <div className="bg-white rounded-3xl border border-gray-200 backdrop-blur-xl w-[320px] overflow-hidden">
        {/* Note Header with Custom Yellow Theme */}
        <div 
          className="flex items-center justify-between p-4 border-b cursor-move"
          style={{ 
            backgroundColor: '#FFF097',
            borderBottomColor: 'rgba(55, 40, 4, 0.2)'
          }}
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-medium text-sm" style={{ color: '#372804' }}>{author}</h3>
              <p className="text-xs opacity-70" style={{ color: '#372804' }}>{timestamp}</p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            {content.trim() && !isEditing && (
              <button 
                ...
                title="Edit note"
              >
                <Edit2 className="h-4 w-4" style={{ color: '#372804' }} />
              </button>
            )}
            ...
            <button 
              ...
              title="Delete note"
            >
              <Trash2 className="h-4 w-4" style={{ color: '#372804' }} />
            </button>
          </div>
        </div>

        {/* Note Content Area with Custom Yellow Theme */}
        <div 
          className="p-4"
          ...
          {isEditing ? (
            <div className="relative">
              <textarea
                ...
                style={{ 
                  color: '#372804',
                  backgroundColor: isOverLimit ? '#FEF2F2' : 'rgba(255, 255, 255, 0.8)',
                  borderColor: isOverLimit ? '#FCA5A5' : 'rgba(55, 40, 4, 0.3)'
                }}
                disabled={isSaving}
              />
              ...
              {wordCountError && (
                <motion.div
                  ...
                  className="absolute -bottom-8 left-0 right-0 text-xs text-red-600 bg-red-50 px-2 py-1 rounded border border-red-200"
                >
            ...
            <div 
              className="min-h-[6rem] p-3 text-sm whitespace-pre-wrap break-words cursor-pointer rounded-lg border border-transparent hover:border-gray-200 transition-colors"
              style={{ color: '#372804' }}
              onClick={() => onEdit?.()}
            >
              {noteContent || "Click to start a thread..."}
            </div>
          )}
        </div>

        {/* Comments Section with Yellow Theme */}
        {!isEditing && (
          <div 
            className="border-t"
            style={{ 
              borderTopColor: 'rgba(55, 40, 4, 0.2)',
              backgroundColor: '#FFFBED'
            }}
          >
            {/* Comments Header */}
            <div className="px-4 py-3 border-b" style={{ borderBottomColor: 'rgba(55, 40, 4, 0.1)' }}>
              <div className="flex items-center space-x-2">
                <MessageSquare className="h-4 w-4" style={{ color: '#372804' }} />
                <span className="text-sm font-medium" style={{ color: '#372804' }}>
                ...
                <div className="flex-1 space-y-2">
                  <textarea
                    ...
                    style={{ 
                      ...
                      borderColor: 'rgba(55, 40, 4, 0.3)'
                    }}
                    rows="2"
                  />
                  <div className="flex items-center justify-between">
                    {/* Removed "Press Ctrl + Enter to comment" for cleaner dashboard */}
                    <span className="text-xs" style={{ color: 'rgba(55, 40, 4, 0.7)' }}>
                    ...
                    <button
                      ...
                      style={{ 
                        backgroundColor: newComment.trim() && !isSubmitting ? '#372804' : '#9CA3AF'
                      }}
                    >
                      {isSubmitting ? (
                        <motion.div
...
// Yellow-themed Comment Item Component
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <span className="text-sm font-medium" style={{ color: '#372804' }}>{comment.author}</span>
            <span className="text-xs ml-2" style={{ color: 'rgba(55, 40, 4, 0.7)' }}>{comment.timestamp}</span>
          </div>
        </div>
        {isOwner && (
          <button
      ...
      <p className="text-sm leading-relaxed" style={{ color: '#372804' }}>{comment.content}</p>
      ...
      {comment.replies && comment.replies.length > 0 && (
        <div className="ml-4 space-y-2 border-l-2 pl-3" style={{ borderLeftColor: 'rgba(55, 40, 4, 0.2)' }}>
